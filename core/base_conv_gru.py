...
class ConvGRUCell(nn.Module):
    ...
    def forward(self, input_tensor, h_cur, mask=None):
        ...
        """
        h_cur=h_cur.type(torch.float32)
...
class Encoder_z0_ODE_ConvGRU(nn.Module):
    ...
    def forward(self, input_tensor , time_steps, mask=None, tracker=None): # dynamic tensor后加
    
        if not self.batch_first:
            # (t, b, c, h, w) -> (b, t, c, h, w)
            input_tensor = input_tensor.permute(1, 0, 2, 3, 4)
        
        assert (input_tensor.size(1) == time_steps.shape[-1] ) #len(time_steps)"Sequence length should be same as time_steps"
    ...
    def run_ode_conv_gru(self, input_tensor , mask, time_steps, run_backwards=True, tracker=None): #dynamic_tensor
        ...
        prev_t=prev_t.unsqueeze(1).unsqueeze(2).unsqueeze(3).to(device)
        t_i=t_i.unsqueeze(1).unsqueeze(2).unsqueeze(3).to(device)
        latent_ys = []
        ...
        for idx, i in enumerate(time_points_iter):
            ...
            yi_ode = ode_sol[:, -1, :] #+ dynamic_tensor[:,i,:]
            xi = input_tensor[:, i, :]
            
            # only 1 now
            yi = self.cell_list[0](input_tensor=xi,
                                   h_cur=yi_ode,
                                   mask=None)#mask=mask[:, i])
